	...
	with ndjson_path.open("r", encoding="utf-8") as f:
		for line in f:
			...
			try:
				...
				continue
			if isinstance(obj, dict) and isinstance(obj.get("describe"), str) and obj["describe"].strip():
				count += 1
	return count


def save_image(img: Image.Image, out_dir: Path, stem: str, index: int, fmt: str) -> Path:
...
def build_arg_parser() -> argparse.ArgumentParser:
	p = argparse.ArgumentParser(description="Generate images from synthesis NDJSON using Qwen-Image")
	p.add_argument("--input", "-i", required=True, help="Path to NDJSON file produced by synthesis")
	p.add_argument("--out", "-o", required=True, help="Destination folder for generated images")
	p.add_argument("--model-path", default=None, help='HF model id or path (default: "Qwen/Qwen-Image")')
	p.add_argument(
		"--aspect-ratio",
	...
	p.add_argument("--steps", type=int, default=8, help="Number of inference steps (default: 8)")
	p.add_argument("--seed", type=int, default=0, help="Seed for reproducibility (used when --no-randomize-seed)")
	p.add_argument("--randomize-seed", action="store_true", help="Use a random seed per image (default)")
	p.add_argument("--no-randomize-seed", dest="randomize_seed", action="store_false", help=argparse.SUPPRESS)
	p.set_defaults(randomize_seed=True)
	p.add_argument("--guidance-scale", type=float, default=1.0, help="Guidance scale (true_cfg_scale) for model")
	p.add_argument("--format", choices=["jpg", "png"], default="jpg", help="Output image format (default: jpg)")

	# Prompt enhancement options (optional)
	p.add_argument("--enhance", action="store_true", help="Enhance each describe via LLM before generation")
	p.add_argument("--enhance-system-prompt", default=SYS_PROMPT, help="System prompt to steer enhancement (default: SYS_PROMPT)")
	p.add_argument("--enhance-model", default=None, help="Model to use for enhancement (default: ENV OBJH_MODEL)")
	p.add_argument("--enhance-base-url", default=None, help="Custom API base URL for enhancement (default: ENV OBJH_API_BASE)")
	p.add_argument("--enhance-temperature", type=float, default=0.3, help="Temperature for enhancement (default: 0.3)")
	p.add_argument("--enhance-max-tokens", type=int, default=256, help="Max tokens for enhancement (default: 256)")
	return p
